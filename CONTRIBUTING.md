...
1. Confirm your recipe behaves as expected:
    - **For recipes that target the browser**, from the root folder run:
        ```shell
        npx parcel recipes/MyNewRecipe/web/index.html
        ```
    - **For recipes that target Node**, from the root folder run:
        ```shell
        npx spago -x recipes/MyNewRecipe/spago.dhall run -m MyNewRecipe.Main
        ```
